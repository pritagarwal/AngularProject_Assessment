...
export class ViewAssessmentComponent implements OnChanges {
  currentPage: number = 1;
  assessmentsPerPage: number = 9;
  searchTerm: string = ''; 
  ...
  //#endregion
  // Holds the filtered list based on search
  filteredAssessments: Assessment_Prit[] = [...this.assessments];

  // Method to filter assessments by search term
  filterAssessments() {
    ...
    if (this.searchTerm === '') {
      ...
      this.filteredAssessments = this.assessments.filter(assessment =>
        assessment.name.toLowerCase().startsWith(this.searchTerm)
      );
    }
  }
  ...
  // Pagination logics
  get arrAssessments(): Assessment_Prit[] {
  ...
  viewSpecialPrice:number=0;
  // storePriceDrop(price:any)
  // {
  //   console.log(price);
  //    this.viewSpecialPrice=price;
